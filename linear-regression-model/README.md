...
In the above formula, `alpha` is the learning rate that we provided earlier in our code as the argument to the `tf.train.GradientDescentOptimizer` class. This process of updating and calculating the loss happens till the derivative of the loss function is as close to `0` as possible. In our case, `m` and `c` are updated independently and simultaneously. This is how gradient descent works.